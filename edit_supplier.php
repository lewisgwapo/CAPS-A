<?php
// edit_supplier.php - Edit supplier information
session_start();
...
// Mock supplier data
$mockSuppliers = [
    ['id' => 1, 'name' => 'PharmaCorp Ltd', 'contact' => 'John Smith', 'phone' => '+1-555-0101', 'email' => 'john@pharmacorp.com', 'address' => '123 Medical Ave, City', 'status' => 'Active'],
    ['id' => 2, 'name' => 'MediSupply Inc', 'contact' => 'Sarah Johnson', 'phone' => '+1-555-0102', 'email' => 'sarah@medisupply.com', 'address' => '456 Health St, Town', 'status' => 'Active'],
    ['id' => 3, 'name' => 'Global Health Pharma', 'contact' => 'Mike Chen', 'phone' => '+1-555-0103', 'email' => 'mike@globalhealthpharma.com', 'address' => '789 Wellness Rd, Metro', 'status' => 'Inactive'],
];
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        if ($conn) {
            $stmt = $conn->prepare(
                "UPDATE suppliers SET supplier_name=?, contact_person=?, phone=?, email=?, address=?, status=? WHERE supplier_id=?"
            );
            ...
            if ($stmt->execute()) {
                ...
                $supplier = ['id' => $supplier_id, 'name' => $name, 'contact' => $contact_person, 'phone' => $phone, 'email' => $email, 'address' => $address, 'status' => $status];
            } else {
                $message = "❌ Error updating supplier. Try again.";
                $message_type = "error";
            }
        } else {
            $message = "✅ Supplier updated successfully! (Mock - database not connected)";
            $message_type = "success";
            $supplier = ['id' => $supplier_id, 'name' => $name, 'contact' => $contact_person, 'phone' => $phone, 'email' => $email, 'address' => $address, 'status' => $status];
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>Edit Supplier | Bowl of Hygea Pharmacy</title>
        ...
        <div class="bg-white rounded-xl shadow p-8 max-w-2xl">
            <form method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="contact_person" value="<?= htmlspecialchars($supplier['contact']) ?>"
                ...
                <div>
                    ...
                    <input type="tel" name="phone" required value="<?= htmlspecialchars($supplier['phone']) ?>"
                ...
                <div>
                    ...
                    <textarea name="address" rows="3" placeholder="e.g., 123 Medical Ave, City"
                              class="w-full border-2 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition"><?= htmlspecialchars($supplier['address']) ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="status" value="Active" <?= $supplier['status'] === 'Active' ? 'checked' : '' ?> class="w-4 h-4">
                            <span class="text-gray-700">Active</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="status" value="Inactive" <?= $supplier['status'] === 'Inactive' ? 'checked' : '' ?> class="w-4 h-4">
